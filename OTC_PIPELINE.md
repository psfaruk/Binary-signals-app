...
### MODEL-RUN-FIX (2026-09-12) — "মডেল রান হয়নি, ১ ঘন্টা অপেক্ষা" রিপোর্টের গভীর ফিক্স

**মূল কারণ (৩টি স্তর):**

1. **requirements.txt এ numpy + scikit-learn ছিল না** — স্যান্ডবক্সে গ্লোবালি
   ইনস্টল থাকায় লোকাল টেস্ট সব পাস করত, কিন্তু Railway এ `SKLEARN_OK=False`
   → প্রতিটি ট্রেইনিং চেষ্টা "sklearn unavailable" → কোনো মডেল কখনো
   রেজিস্টার হতেই পারত না। এখন দুটোই requirements.txt এ পিন করা (টেস্ট
   T9 এ এই রিগ্রেশনের স্থায়ী গার্ড)।
2. **সাইলেন্ট ফেইলিউর** — ডেমন ফেইল করলে UI তে কিছুই দেখাত না। এখন:
   * `bootstrap_status()` এ `blocked` / `sklearn_ok` / প্রতি-পেয়ার
     পার্সিস্টেন্ট স্টেট (candles, status, rows, t1/t2 accuracy, fetch_error,
     reason, version)।
   * `GET /api/prediction/overview` — মডেল ট্যাবের এক-কল পেলোড (ডেমন +
     রেজিস্ট্রি + ক্যান্ডেল কাউন্ট + গ্রেডেড রেজাল্ট)।
3. **রিট্রাই গ্যাপ** — খালি রানের পর পরবর্তী চেষ্টা ৬ ঘন্টা পরে হতো। এখন
   হার্ডকোডেড অ্যাডাপটিভ: রেজিস্টার হলে ৬ ঘন্টা, **না হলে/ত্রুটিতে ১০ মিনিট**।
   নতুন টোকেন পেস্ট করলে `notify_token_pushed()` ডেমনকে **সাথে সাথে**
   জাগিয়ে দেয় (feed-এর wake প্যাটার্নের মতোই)।

**নতুন "মডেল" ট্যাব (frontend):** ডেমন স্ট্রিপ (চলছে/শেষ রান/পরবর্তী রান/
রেজিস্টার্ড সংখ্যা + "এখনই ট্রেইন করুন" বাটন), পেয়ার-টেবিল (কোন পেয়ার কত
ক্যান্ডেল, কী স্ট্যাটাস — ✅ভেরিফায়েড/প্রোভিশনাল/ডেটা কম/ব্লকড/ত্রুটি, কত
রো ট্রেইন, T+1/T+2 walk-forward accuracy বনাম baseline, ভার্সন), এবং
মডেলের আসল রেজাল্ট — প্রতিটি ফ্রিজ করা প্রেডিকশনের দিক-নির্ভুলতা
(prediction_analytics এ `dir_*` ফিল্ড; emit হোক বা না হোক সব গ্রেড হয়)।

**লাইভ প্রমাণ (আসল টোকেন দিয়ে, প্রোডাকশন-কোল্ড-স্টার্ট শেপে):**
* ৯ পেয়ার **৫৩.৭ সেকেন্ডে** ট্রেইন + রেজিস্টার (বাজেট ৫–৭ মিনিট)।
* টোকেন expire থাকলে: ফেচ প্রতি-পেয়ারে `connect_failed (AUTH REJECT)`
  রেকর্ড হয়, ট্যাবে দেখা যায়, টোকেন মডাল খোলে, ১০ মিনিট পর পর চেষ্টা —
  নতুন টোকেন দিলেই সাথে সাথে ট্রেইনিং।
* টেস্ট: test_fast_train **৩৪/৩৪**, test_otc_predict ৪২/৪২,
  test_otc_pipeline ২২/২২, target-gate ALL PASS, every-candle PASS,
  replay ১৭,৭১৮ graded healthy।

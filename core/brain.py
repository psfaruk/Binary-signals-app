...
def init_brain():
    ...
    try:
        ...
        try:
            ...
            print(f"[silent-except] core/brain.py:315 {type(_e).__name__}: {_e}")
            pass
        raise
    finally:
        conn.close()
...
def _derive_signal_type(modules: dict, reasons: list) -> str:
    ...
    try:
        ...
        print(f"[silent-except] core/brain.py:362 {type(_e).__name__}: {_e}")
        pass
...
def record_prediction(prediction: dict, asset: str, period: int,
    ...
    try:
        ...
        try:
            ...
            if _total_groups > 0:
                # Conservative max-per-group weight is 5.0.
...
def analyze_and_learn(min_samples: int = 20):
    ...
    try:
        ...
        try:
            ...
            print(f"[silent-except] core/brain.py:880 {type(_e).__name__}: {_e}")
            pass
        raise
    finally:
        conn.close()
...
def _analyze_loss_clusters(cur):
    ...
    # Post-loop check for a streak still in progress at end of dataset.
    if current_asset is not None and current_streak >= _MIN_LOSS_STREAK:

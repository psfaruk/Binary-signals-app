...
PROBLEM: Quotex silently drops ticks when >15 pairs are subscribed.
User wants ALL pairs streaming smoothly without limits.

SOLUTION: 3-tier rotating streaming system:

Tier 1: ACTIVE STREAMS (15 pairs, always-on)
- Highest-payout pairs currently being predicted
- Full candle data + tick streaming
- Receives all ticks in real-time

Tier 2: WARM CACHE (next 15 pairs, periodic refresh)
- Subscribed for 60s every 5 minutes
- Gets candle snapshots (OHLC) without full tick stream
- Lower bandwidth, no rate-limit risk

Tier 3: COLD STORAGE (remaining pairs)
- Not subscribed
- Candle history fetched on-demand when user opens the pair
- No streaming cost

ROTATION LOGIC:
- Every 5 minutes, rotate pairs between tiers
- Pairs with recent signals stay in Tier 1
- Pairs user is viewing get priority boost to Tier 1
- Pairs with good time-of-day patterns get scheduled

This gives the appearance of "all pairs streaming" while staying
within Quotex's 15-pair concurrent limit.
"""
import os
import time
import asyncio
...
# Pairs that should ALWAYS be in Tier 1 (user favorites or all-time OTC)
...
class SmartStreamingManager:
    ...
    Usage:
        manager = SmartStreamingManager(feed)
        await manager.start()  # starts rotation loop

        # When user opens a pair:
        manager.boost_to_tier1("EURUSD_otc")

        # When prediction is needed:
        manager.mark_active("EURUSD_otc")  # keeps in Tier 1
    """
    ...
    def _promote_to_tier1(self, pair: str):
        ...
        if len(self.tier1_pairs) >= TIER1_MAX_STREAMS:
            # Find lowest-priority pair to evict
    ...
    def _find_eviction_candidate(self) -> Optional[str]:
        ...
        # Don't evict viewing pairs or always-tier1 pairs
        candidates = [
        ...
        if not candidates:
            return None
        # Sort by last_active (oldest first = evict first)
    ...
    async def _rotate_tiers(self):
        ...
        # Find tier1 pairs that haven't been active recently
        to_demote = [
            p for p in self.tier1_pairs
        ...
        # Find tier2/tier3 pairs that should be promoted
        # (high payout, good time-of-day patterns, or recently active)
        to_promote = []
        ...
        # Sort promotes by last_active (most recent first)
        to_promote.sort(key=lambda p: self.last_active.get(p, 0), reverse=True)

        # Swap: demote oldest, promote newest
    ...
    async def _refresh_tier2(self):
        ...
        then unsubscribe. This keeps their candle data fresh."""
        if not self.tier2_pairs:
            return

        for pair in list(self.tier2_pairs):
            try:
                # Check if we have capacity (don't exceed tier1_max + 2 buffer)

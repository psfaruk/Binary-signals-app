"""
Microstructure + helpers for the Binary Signals App.

This file used to contain a 4200-line theory-blend engine (CON, REV, RUN,
TRAP, GAP, RNG, MST, MICRO, MEAN, VELOCITY, LIVE_WICK, ORDERFLOW, HOLD,
MOMENTUM, CONTINUITY, HISTORY, OB, SWEEP, STRUCT, ZZ, plus the dead
LAST / SHIFT / FVG). As of 2026-07-13 the prediction path was switched to
``candle_reaction.predict_from_candle`` (pure price-action reaction engine)
and the theory engine stopped being called. The dead theories and all the
supporting market-state classifiers, caches, EMA/RSI helpers, etc. have
been removed.

What remains here is the small set of helpers that ARE still imported by
``feed.py`` and ``sim_feed.py``:

  * ``_round_level``   — proximity of a price to a psychological round number
  * ``_key_levels``    — recent swing highs/lows (S/R levels)
  * ``_parse_votes``   — legacy parser for ``"CODE:+N CALL"`` reason strings
                         (kept so the postmortem logger keeps working; on
                         candle_reaction reasons it harmlessly returns [])
  * ``_atr``           — True Range ATR
  * ``_build_micro``   — full tick microstructure dict (used by sim_feed.py
                         for the micro input to predict_from_candle)

Everything else is gone. If you need the old theory engine back, recover it
from git history (commit prior to this refactor).
"""
import math
...
def _parse_votes(reasons):
    ...
    Kept for backward compatibility with the postmortem logger in feed.py /
    sim_feed.py. On candle_reaction reasons (which use plain prose like
    '5+ UP streak → PUT reversal (75% win rate)') this returns [] because
    the reasons don't have the ``CODE:+N DIRECTION`` format. That's fine —
    the logger's ``fired`` / ``right`` / ``wrong`` sets just stay empty.
    """
    votes = []
    for r in reasons:
        if ":" not in str(r):
...
def _atr(candles, n=20):
    ...
    Falls back to 0.0001 on flat / single-candle inputs to avoid
    divide-by-zero in callers.
    """
    if not candles:
        return 0.0001
...
def _build_micro(ticks, open_price):
    ...
    Used by sim_feed.py as the ``micro`` input to
    ``candle_reaction.predict_from_candle``. (feed.py has its own
    ``_analyze_microstructure`` that produces a similar dict inline.)
    ...
    # FIX (refactor 2026-07-14): the old code reassigned `n` here to
    # ``len(abs_deltas_sorted)`` (the count of non-zero tick deltas),
    # which corrupted the ``tick_count`` field in the return dict — every
    # theory that read ``micro["tick_count"]`` got the delta count instead
    # of the tick count. Renamed the local to ``n_deltas`` so the outer
    # ``n`` (real tick count) is preserved.
    orderflow = None
    if n >= 12:
        deltas = []
        for i in range(1, n):
